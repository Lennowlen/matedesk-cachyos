...
# Matikan proses lama & bersihkan socket X11
...
# 2. Start Termux-X11 Display Server on :1
...
# 5. Launch XFCE4 Desktop Session di Arch Linux on :1
echo -e "${GREEN}[4/4] Membuka sesi CachyOS Desktop...${NC}"
proot-distro login archlinux --shared-tmp -- env DISPLAY=:1 PULSE_SERVER=127.0.0.1 GALLIUM_DRIVER=virpipe MESA_GL_VERSION_OVERRIDE=3.3 MESA_GLSL_VERSION_OVERRIDE=330 LIBGL_ALWAYS_INDIRECT=0 VIRGL_CLIENT_DIR=/data/data/com.termux/files/usr/tmp dbus-launch --exit-with-session xfce4-session
